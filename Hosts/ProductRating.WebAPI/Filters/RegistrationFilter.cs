...
namespace ProductRating.WebAPI.Filters
{
    ...
    {
        private readonly RegistrationFilterOptions _options;
        private readonly IErrorDTOService _errorDTOService;

        public RegistrationFilter(IOptions<RegistrationFilterOptions> options, IErrorDTOService errorDTOService)
        {
        ...
        {
            if (!context.ActionArguments.TryGetValue("request", out var argument) || argument is not RegistrationRequest request)
            {
        ...
        private List<string> Validate(RegistrationRequest request)
        {
            ...
            {
                errors.Add("Name пустой или null.");
            }
            else if (request.Name.Length < _options.NameMinLength || request.Name.Length > _options.NameMaxLength)
            {
                errors.Add("Недопустимая длина Name.");
            }
